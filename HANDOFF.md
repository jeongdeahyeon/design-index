      ...
      "alts": [                            // 도면안들 (ALT-A, ALT-B…)
        {
          ...
          "images": [
            {
              "cat": "평면도",              // 카테고리 (뷰어 하단 탭으로 필터). 자유 문자열이지만 CAT_ORDER 순서대로 자동정렬(아래)
              "caption": "ALT-B 전체 평면",
...
### 카테고리(`cat`) 자동 정렬
`index.html`의 `CAT_ORDER` 배열 순서대로 이미지가 자동 정렬됨(현장·도면안마다 수동으로 순서 맞출 필요 없음):
`평면도 → 실사 → 메인컷 → 현관 → 주방 → 주방발코니 → 거실 → 안방 → 거실욕실 → 안방욕실`
목록에 없는 카테고리는 뒤에 가나다 역순으로 붙음. 같은 카테고리끼리는 원래(업로드/배열) 순서 유지(안정 정렬).
`admin.html`의 카테고리 입력칸 자동완성(`catlist`)도 이 이름들로 맞춰져 있음 — 새 카테고리 지을 때 그대로 쓰면 순서 신경 안 써도 됨.
정렬은 뷰어 렌더링 시점에만 적용되고 `projects.json` 자체는 안 바뀜(관리자의 ▲▼는 같은 카테고리 안에서의 우선순위를 정할 때만 씀).
